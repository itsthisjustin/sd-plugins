...
3. **Register it** in the hosted catalog (this makes it installable from the
   Plugin Store): the catalog is maintained outside this repo — ask the
   maintainer to add a `plugins[]` entry with `name`, `title`, `description`,
   `author`, `version`, `base`
   (`https://raw.githubusercontent.com/itsthisjustin/sd-plugins/main/<name>/`),
